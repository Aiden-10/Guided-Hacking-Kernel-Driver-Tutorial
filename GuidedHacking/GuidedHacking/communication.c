...
NTSTATUS IoControl(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
	...
	if (ControlCode == IO_GET_CLIENTADDRESS)
	{
		...
		*OutPut = base_address_of_target_application; // assign the output system buffer then value of the game address
	...
	{
		PULONG OutPut = (PULONG)Irp->AssociatedIrp.SystemBuffer;
		*OutPut = targetProcessId; // assign the output system buffer then value of the game address
		...
		if (NT_SUCCESS(PsLookupProcessByProcessId(ReadInput->ProcessId, &Process))) {
			...
			Status = STATUS_SUCCESS;
			ByteIO = sizeof(KERENEL_READ_REQUEST);
